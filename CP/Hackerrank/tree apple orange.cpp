...
int apple_Count=0,oranges_count=0;

for(long int i=0;i<apples.size();i++)
{

    if(apples[i]<0)
    {
      if (s <= (a - abs(apples[i])) && (a - abs(apples[i]))<=t)
      {
          apple_Count++;
      }
    }
...
for(long int i=0;i<oranges.size();i++)
{
    if(oranges[i]>0)
    {
    ...
    {
        if(t>=(b-abs(oranges[i]))&&s<=(b-abs(oranges[i])))
            oranges_count++;
    }

}
cout << apple_Count << endl << oranges_count;
}
